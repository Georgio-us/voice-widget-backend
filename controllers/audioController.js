...
const formatCardForClient = (req, p) => {
  ...
  const images = Array.isArray(p.images)
    ? p.images
        .map((v) => String(v || '').trim())
        .filter(Boolean)
        .map((v) => v.replace('https://<backend-host>', baseUrl))
    : [];
...
const buildLocationLexicon = (rows = []) => {
  ...
  const aliases = [
    ...
    ['эликанте', 'Alicante'], ['эликанты', 'Alicante'], ['эльканте', 'Alicante'], ['еликанте', 'Alicante'],
    ['бенидорм', 'Benidorm'],
...
const parseBudgetTokenToInt = (rawNum, rawSuffix = '') => {
  ...
  if (!Number.isFinite(value)) return null;
  const suffix = String(rawSuffix || '').toLowerCase();
  if (/(млн|миллион|миллиона|миллионов|million|millions|mio|лям|ляма|лямов)/i.test(suffix)) {
    value *= 1000000;
  } else if (/(k|к|тыс|тысяч|thousand|mil)/i.test(suffix) && value < 10000) {
    value *= 1000;
  }
  value = Math.round(value);
...
const detectBudgetBoundsFromText = (text = '') => {
  ...
  // conversational half-million forms
  if (/(полу?[\s-]*миллион|пол[\s-]*лям)/i.test(s)) {
    const upperHalf = s.match(/(?:до|up\s*to|hasta|max(?:imum)?|не\s*более)\s*(?:полу?[\s-]*миллион|пол[\s-]*лям)/i);
    if (upperHalf) return { mode: 'upper', max: 500000 };
    const lowerHalf = s.match(/(?:от|from|desde|min(?:imum)?|не\s*менее)\s*(?:полу?[\s-]*миллион|пол[\s-]*лям)/i);
    if (lowerHalf) return { mode: 'lower', min: 500000 };
    return { mode: 'single', min: 500000 };
  }

  if (/(^|\s)лям(?:а|ов)?($|\s)/i.test(s)) {
    const upperM = s.match(/(?:до|up\s*to|hasta|max(?:imum)?|не\s*более)\s*лям(?:а|ов)?/i);
    if (upperM) return { mode: 'upper', max: 1000000 };
    const lowerM = s.match(/(?:от|from|desde|min(?:imum)?|не\s*менее)\s*лям(?:а|ов)?/i);
    if (lowerM) return { mode: 'lower', min: 1000000 };
    return { mode: 'single', min: 1000000 };
  }

  const rangeDash = s.match(/(\d[\d\s.,]{0,10})\s*(k|к|тыс\.?|тысяч|thousand|mil(?:es)?|eur|euro|euros|евро|€|млн|миллион(?:а|ов)?|million|mio|лям(?:а|ов)?)?\s*[-–—]\s*(\d[\d\s.,]{0,10})\s*(k|к|тыс\.?|тысяч|thousand|mil(?:es)?|eur|euro|euros|евро|€|млн|миллион(?:а|ов)?|million|mio|лям(?:а|ов)?)?/i);
  if (rangeDash) {
  ...
  const rangeFromTo = s.match(/(?:от|from|desde)\s*(\d[\d\s.,]{0,10})\s*(k|к|тыс\.?|тысяч|thousand|mil(?:es)?|eur|euro|euros|евро|€|млн|миллион(?:а|ов)?|million|mio|лям(?:а|ов)?)?\s*(?:до|to|a|hasta)\s*(\d[\d\s.,]{0,10})\s*(k|к|тыс\.?|тысяч|thousand|mil(?:es)?|eur|euro|euros|евро|€|млн|миллион(?:а|ов)?|million|mio|лям(?:а|ов)?)?/i);
  if (rangeFromTo) {
  ...
  const upper = s.match(/(?:до|up\s*to|hasta|max(?:imum)?|не\s*более)\s*(\d[\d\s.,]{0,10})\s*(k|к|тыс\.?|тысяч|thousand|mil(?:es)?|eur|euro|euros|евро|€|млн|миллион(?:а|ов)?|million|mio|лям(?:а|ов)?)?/i);
  if (upper) {
    const v = parseBudgetTokenToInt(upper[1], upper[2] || '');
    if (Number.isInteger(v)) return { mode: 'upper', max: v };
  }

  const lower = s.match(/(?:от|from|desde|min(?:imum)?|не\s*менее)\s*(\d[\d\s.,]{0,10})\s*(k|к|тыс\.?|тысяч|thousand|mil(?:es)?|eur|euro|euros|евро|€|млн|миллион(?:а|ов)?|million|mio|лям(?:а|ов)?)?/i);
  if (lower) {
    const v = parseBudgetTokenToInt(lower[1], lower[2] || '');
    if (Number.isInteger(v)) return { mode: 'lower', min: v };
  }

  const singleMatch = s.match(/(\d[\d\s.,]{0,10})\s*(k|к|тыс\.?|тысяч|thousand|mil(?:es)?|млн|миллион(?:а|ов)?|million|mio|лям(?:а|ов)?)?/i);
  if (singleMatch) {
...
const extractInsightsWithLLM = async (session, newMessage, locationLexicon = []) => {
  ...
  try {
    ...
    if (budgetBounds) {
      if (budgetBounds.mode === 'upper') delete sanitized.budgetMin;
      if (budgetBounds.mode === 'lower') delete sanitized.budgetMax;
...
const sanitizeNoInventoryClaim = (text = '', execution = null) => {
  ...
  const noInventoryPatterns = [
    ...
    /не входит в наш основной каталог/i,
    /не входит в основной каталог/i
  ];
  if (!noInventoryPatterns.some((re) => re.test(raw))) return raw;

  const inventorySafeText = raw.replace(
  ...
  if (inventorySafeText !== raw && !isSupportedGeo) return inventorySafeText;

  if (isFallbackGeo) {
    return inventorySafeText.replace(
      /нет доступных объектов/gi,
  ...
  return raw.replace(
    ...
    /[^.?!]{0,140}не входит в наш основной каталог\.?\s*/i,
    'В этом направлении есть доступные объекты в текущем каталоге. '
  ).replace(
    /[^.?!]{0,140}не входит в основной каталог\.?\s*/i,
    'В этом направлении есть доступные объекты в текущем каталоге. '
  ).replace(
    /[^.?!]{0,160}нет объектов в этом направлении\.?\s*/i,
    'В этом направлении есть доступные объекты в текущем каталоге. '
  ).replace(
    /нет доступных объектов/i,
