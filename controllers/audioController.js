...
const detectCardIntent = (text = '', lang = '') => {
  ...
  // EN: "show", "show me", "cards" (при включённом английском)
  const enShow = /\b(show\s*(me)?|show\s+(them|it)|cards?)\b/i;
  // ES: "muestra", "mostrar", "ver tarjetas", "tarjeta" (при включённом испанском)
  const esShow = /\b(muestra|mostrar|enseña|enseñar|ver\s*(las?)?\s*tarjetas?|tarjeta)\b/i;
  let isShow = ruShow.test(t);
  if (langCode === 'en') isShow = isShow || enShow.test(t);
  if (langCode === 'es') isShow = isShow || esShow.test(t);
...
const detectVerbalSelectIntent = (text = '') => {
  ...
  if (!t) return false;
  // Предохранитель: "покажи" / "show" / "muestra" — это show-intent, не выбор
  if (/(покажи(те)?|показать|посмотреть|\bshow\b|muestra|mostrar|enseña|enseñar)/i.test(t)) return false;
...
Продолжай диалог естественно, но соблюдай эти ограничения.`;
    ...
    // 🔎 Детектор намерения/вариантов (lang: RU/EN/ES — триггер слайдера "покажи"|"show"|"muestra")
    const { show, variants } = detectCardIntent(transcription, getPrimaryLanguage(session));
