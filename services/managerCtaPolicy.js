...
const hasObjectFollowUpIntent = (text = '') => {
  return /(подробнее|детальн|дешевле|дешевш|дороже|дорожч|еще|ещё|ще|вариант|варіант|объект|об'єкт|квартир|дом|будин|что\s+дальше|що\s+далі|как\s+посмотреть|як\s+подивит|можно\s+посмотреть|можна\s+подивит)/i.test(text);
};
